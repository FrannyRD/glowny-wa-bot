...
// ✅ MODO CONVERSACIONAL (NUEVO)
// Responde con tono natural y guiado, sin romper catálogo/carrito/ubicación/CRM/Chatwoot.
const CONVERSATIONAL_MODE = String(process.env.CONVERSATIONAL_MODE || "true")
  .trim()
  .toLowerCase() !== "false";
const CONVERSATIONAL_AI_ENABLED = String(process.env.CONVERSATIONAL_AI_ENABLED || "true")
  .trim()
  .toLowerCase() !== "false";
const OPENAI_MODEL = process.env.OPENAI_MODEL || "gpt-4o-mini";
const OPENAI_TIMEOUT_MS = Number(process.env.OPENAI_TIMEOUT_MS || 6500);
...
// =============================
// ✅ MODO CONVERSACIONAL NATURAL (NUEVO)
// - Capa extra encima del flujo existente.
// - No reemplaza carrito, ubicación, Chatwoot, Bothub, recordatorios ni modo humano.
// - Primero usa reglas seguras + catálogo real; OpenAI solo redacta cuando hay API key y no hay una respuesta cerrada.
// =============================
function formatRDPrice(value) {
  const numberValue = Number(value);
  if (!Number.isFinite(numberValue) || numberValue <= 0) return "";
  try {
    return `RD$${numberValue.toLocaleString("es-DO")}`;
  } catch (_) {
    return `RD$${numberValue}`;
  }
}

function cleanOutboundText(text, maxLen = 1200) {
  let out = String(text || "").replace(/\r/g, "").trim();
  out = out.replace(/\n{4,}/g, "\n\n");
  if (out.length > maxLen) out = out.slice(0, maxLen - 3).trim() + "...";
  return out;
}

function hasAnyNormalized(text, phrases) {
  const normalized = normalizeMatchText(text);
  if (!normalized) return false;
...
function isAvailabilityIntentText(text) {
  return hasAnyNormalized(text, [
    "disponible",
    "disponibilidad",
    "tienen",
...
function isUsageIntentText(text) {
  return hasAnyNormalized(text, [
    "como se usa",
    "cómo se usa",
    "modo de uso",
...
function isIngredientIntentText(text) {
  return hasAnyNormalized(text, [
    "ingrediente",
    "ingredientes",
    "contiene",
...
function isWarningIntentText(text) {
  return hasAnyNormalized(text, [
    "advertencia",
    "advertencias",
    "contraindicacion",
...
function isDurationIntentText(text) {
  return hasAnyNormalized(text, [
    "cuanto dura",
    "cuánto dura",
    "duracion",
...
function isDeliveryIntentText(text) {
  return hasAnyNormalized(text, [
    "envio",
    "envío",
    "delivery",
...
function isHumanHelpIntentText(text) {
  return hasAnyNormalized(text, [
    "asesor",
    "asesora",
    "representante",
...
function isComplaintIntentText(text) {
  return hasAnyNormalized(text, [
    "problema",
    "reclamo",
    "queja",
...
function scoreProductForUserText(userText, product) {
  ...
  const extraText = normalizeMatchText(
    `${product?.description || ""} ${product?.how_to_use || ""} ${product?.ingredients || ""}`
  );
  const extraOverlap = userTokens.filter((token) => extraText.includes(token));
  score += extraOverlap.length * 5;

  return score;
}

function findCatalogProductsByText(userText, { limit = 5, minScore = 30 } = {}) {
  const text = String(userText || "").trim();
  if (!text || !productSearchTokens(text).length) return [];

  return productIndex
    .map((entry) => ({ product: entry.data, score: scoreProductForUserText(text, entry.data) }))
    .filter((item) => item.score >= minScore)
    .sort((a, b) => b.score - a.score)
    .slice(0, limit);
}

function getSessionAdProduct(session) {
  const adProduct = session?.ad_context?.product || null;
  if (!adProduct) return null;
  const full = lookupProductByRetailerId(adProduct.id) || productIndex.find((p) => String(p.data.id) === String(adProduct.id))?.data;
...
function buildProductPriceReply(product) {
  const price = formatRDPrice(product?.price);
  if (price) {
    return `Sí 💕 *${product.name}* está en ${price}.\n\nPara comprarlo, abre el catálogo, agrégalo al carrito y envíamelo por aquí. Luego te pido la ubicación para coordinar la entrega.`;
  }
  return `Sí 💕 tengo identificado *${product?.name || "ese producto"}*, pero ahora mismo no tengo el precio cargado para darte un monto exacto por aquí.\n\nPuedes enviarlo en el carrito desde el catálogo y una representante confirma disponibilidad, total y entrega.`;
}

function buildProductAvailabilityReply(product) {
  if (product?.in_stock === false) {
    return `Ahora mismo *${product.name}* figura como no disponible en el catálogo.\n\nSi quieres, puedes revisar otras opciones disponibles en el catálogo o escribirnos cuál buscabas para ayudarte mejor 💕`;
  }
  const price = formatRDPrice(product?.price);
  return `Sí, *${product.name}* figura disponible en el catálogo${price ? ` y cuesta ${price}` : ""} ✨\n\nPuedes agregarlo al carrito y enviarlo por aquí para continuar con tu pedido.`;
}
...
function buildProductInfoReply(product, intent) {
  ...
  if (intent === "usage") {
    const use = String(product?.how_to_use || product?.description || "").trim();
    return use
      ? `Claro 💕 Sobre *${name}*:\n\n${use}${price ? `\n\nPrecio: *${price}*.` : ""}`
      : `Claro 💕 *${name}* está en el catálogo${price ? ` a *${price}*` : ""}. Si necesitas una indicación específica de uso, una representante puede confirmártela contigo.`;
  }

  if (intent === "ingredients") {
    const ingredients = String(product?.ingredients || "").trim();
    return ingredients
      ? `Los ingredientes de *${name}* son:\n\n${ingredients}`
      : `De *${name}* no tengo ingredientes cargados en el catálogo ahora mismo. Para evitar darte una información incorrecta, una representante puede confirmarlo contigo 💕`;
  }

  if (intent === "warnings") {
    const warnings = String(product?.warnings || "").trim();
    return warnings
      ? `Importante sobre *${name}*:\n\n${warnings}`
      : `Para *${name}* no tengo advertencias específicas cargadas ahora mismo. Si tienes piel sensible, alergias o una condición particular, mejor lo validamos con una representante antes de comprar 💕`;
  }

  if (intent === "duration") {
    const duration = String(product?.duration_text || "").trim();
    return duration
      ? `Sobre la duración de *${name}*:\n\n${duration}`
      : `La duración de *${name}* puede variar según el uso. Si me envías el producto en el carrito, una representante puede orientarte mejor según tu caso 💕`;
  }

  const description = String(product?.description || "").trim();
  return `${description ? `${description}\n\n` : ""}${buildProductAvailabilityReply(product)}`;
}

function buildProductMatchesReply(matches, userText) {
  const cleanMatches = (matches || []).filter((item) => item?.product).slice(0, 3);
  ...
  const intro = isPriceInquiryText(userText)
    ...
    : "Creo que te pueden servir estas opciones del catálogo 💕";

  return `${intro}:\n\n${lines.join("\n")}\n\nPara comprar, abre el catálogo, agrega el producto al carrito y envíamelo por aquí.`;
}

function buildBuyingInstructionsReply() {
  return `Claro 💕 Para comprar es así:\n\n1️⃣ Abre el *Catálogo de WhatsApp*.\n2️⃣ Agrega los productos al carrito.\n3️⃣ Envía el carrito por aquí.\n4️⃣ Luego compárteme tu ubicación 📍 para coordinar la entrega.\n\nCuando recibamos carrito y ubicación, una representante confirma disponibilidad, total y entrega.`;
}
...
function buildAskProductReply(userText) {
  if (isPriceInquiryText(userText)) {
    return `Claro 💕 ¿De cuál producto quieres saber el precio?\n\nTambién puedes abrir el catálogo, tocar el producto que te interesa y enviarlo en el carrito para confirmarte todo más rápido.`;
  }
  return `Claro 💕 dime el nombre del producto que estás buscando o envíamelo desde el catálogo y te ayudo con precio, disponibilidad o forma de uso.`;
}
...
function buildWelcomeConversationalText(greetingName, referredProduct) {
  const adProductLine = referredProduct?.name
    ? (formatRDPrice(referredProduct.price)
        ? `Veo que vienes por *${referredProduct.name}* ✨\nSu precio es *${formatRDPrice(referredProduct.price)}*.\n\n`
        : `Veo que vienes por *${referredProduct.name}* ✨\n\n`)
    : "";

  return (
    `¡Hola${greetingName}! Bienvenida a Glowny Essentials 💕\n\n` +
    `Te ayudo con tu pedido paso a paso.\n\n` +
    adProductLine +
    `🛍️ Para comprar:\n` +
...
function buildPriceWelcomeConversationalText(greetingName, referredProduct) {
  const priceLine =
    referredProduct?.name && formatRDPrice(referredProduct.price)
      ? `El precio de *${referredProduct.name}* es *${formatRDPrice(referredProduct.price)}*.`
...
async function generateConversationalAiReply({ userText, customerName, matches, referredProduct }) {
  ...
  if (referredProduct?.name) productsForContext.push(referredProduct);
  for (const item of matches || []) {
    if (item?.product && !productsForContext.some((p) => String(p.id) === String(item.product.id))) {
      productsForContext.push(item.product);
    }
  }

  const productContext = productsForContext.slice(0, 5).map((p) => ({
    name: p.name,
    price: p.price,
    in_stock: p.in_stock,
  ...
  try {
        ...
        messages: [
          ...
          },
          {
            ...
            content: JSON.stringify({
              ...
              catalogContext: productContext,
              buyingFlow: "Abrir catálogo de WhatsApp > agregar productos al carrito > enviar carrito por el chat > enviar ubicación > una representante confirma disponibilidad, total, pago y entrega.",
            }),
          },
        ],
      },
    ...
    const answer = cleanOutboundText(res?.data?.choices?.[0]?.message?.content || "", 900);
    if (!answer) return null;
    return answer;
  } catch (e) {
    console.error("⚠️ OpenAI conversacional no respondió:", e?.response?.data || e?.message || e);
    return null;
  }
}

async function buildConversationalReply({ userText, session, customerName }) {
  if (!CONVERSATIONAL_MODE) return null;
  ...
  debugJson("🧠 Conversacional intent", {
    ...
    referredProduct: referredProduct
      ? { id: referredProduct.id, name: referredProduct.name, price: referredProduct.price }
      : null,
  ...
  if (intent === "catalog" || intent === "buy") {
    return {
      ...
      chatwootNote: "BOT: Respuesta conversacional con instrucciones de compra/catálogo.",
    };
  }
  ...
  if (intent === "delivery") {
    return {
      type: "text",
      body: buildDeliveryReply(),
      chatwootNote: "BOT: Respuesta conversacional sobre entrega/ubicación.",
    };
  }
  ...
  if (intent === "payment") {
    return {
      type: "text",
      body: buildPaymentReply(),
      chatwootNote: "BOT: Respuesta conversacional sobre pagos.",
    };
  }
  ...
  if (intent === "thanks") {
    return {
      type: "text",
      body: "Con gusto 💕 Cuando tengas el producto listo, envíame el carrito por aquí y te ayudo a continuar.",
      chatwootNote: "BOT: Respuesta conversacional a agradecimiento.",
    };
  }
  ...
  if (intent === "human" || intent === "complaint") {
    return {
      ...
      body:
        intent === "complaint"
          ? "Entiendo 💕 Lamento el inconveniente. Voy a dejar tu conversación para que una representante pueda revisarla y ayudarte mejor por aquí."
          : "Claro 💕 Te dejo con una representante para que pueda ayudarte mejor por aquí.",
      sessionPatch: {
        human_until: Date.now() + 30 * 60 * 1000,
        conversationMode: "human",
      },
      cancelFollowups: true,
      chatwootNote: "BOT: Cliente pidió ayuda humana o reportó inconveniente. Conversación pausada para representante.",
    };
  }
  ...
  if (intent === "price") {
    ...
    if (matches.length) {
      return {
        type: "cta",
        body: buildProductMatchesReply(matches, text),
        buttonText: "🛍️ Ver catálogo",
        url: WHATSAPP_CATALOG_URL,
        chatwootNote: "BOT: Respuesta conversacional con varias opciones de precio.",
      };
    }
    ...
    return {
      ...
      buttonText: "🛍️ Ver catálogo",
      url: WHATSAPP_CATALOG_URL,
      chatwootNote: "BOT: Preguntó precio sin producto claro.",
    };
  }

  if (["availability", "usage", "ingredients", "warnings", "duration"].includes(intent)) {
    if (referredProduct?.name) {
      const body = intent === "availability"
        ? buildProductAvailabilityReply(referredProduct)
        : buildProductInfoReply(referredProduct, intent);
      return {
        type: "cta",
        body,
        buttonText: "🛍️ Ver catálogo",
    ...
    if (matches.length) {
      return {
        ...
        buttonText: "🛍️ Ver catálogo",
        url: WHATSAPP_CATALOG_URL,
        chatwootNote: `BOT: Respuesta conversacional ${intent} con posibles productos.`,
      };
    }
    ...
    return {
      ...
      buttonText: "🛍️ Ver catálogo",
      url: WHATSAPP_CATALOG_URL,
      chatwootNote: `BOT: ${intent} sin producto claro.`,
    };
  }

  if (intent === "product_search") {
    if (matches.length === 1 || (matches[0]?.score || 0) - (matches[1]?.score || 0) >= 18) {
    ...
    return {
      ...
      buttonText: "🛍️ Ver catálogo",
      url: WHATSAPP_CATALOG_URL,
      chatwootNote: "BOT: Varias coincidencias de producto detectadas por conversación.",
    };
  }
  ...
  if (aiReply) {
    return {
      type: "text",
      body: aiReply,
      chatwootNote: "BOT: Respuesta conversacional generada con IA bajo reglas del catálogo.",
    };
  }
...
async function sendConversationalReply({ reply, userPhone, session, customerName }) {
  if (!reply?.body) return false;

  if (reply.sessionPatch && typeof reply.sessionPatch === "object") {
    Object.assign(session, reply.sessionPatch);
  }

  if (reply.cancelFollowups) {
    await cancelReminders(userPhone, session, "conversational_human_or_complaint");
    await cancelFollowupTick(userPhone, session, "conversational_human_or_complaint");
  }

  const body = cleanOutboundText(reply.body, 1200);

  if (reply.type === "cta" && reply.url && reply.buttonText) {
    await sendWhatsAppCtaUrl(userPhone, body, reply.buttonText, reply.url, "BOT", {
      conversational: true,
    });
  } else {
    await sendWhatsAppText(userPhone, body, "BOT", { conversational: true });
  }
  ...
  await sendBotToChatwoot({
    ...
    message: reply.chatwootNote || `BOT: Respuesta conversacional enviada.`,
  });
...
Puedes hacerlo desde el clip 📎 > Ubicación > Enviar. 💗`
      ...
      // ✅ Si es primer mensaje (inbound_text_count === 1), activamos recordatorios también
      // para respuestas conversacionales. Se mantiene el bloque original más abajo para saludos/bienvenida.
      ...
      // ✅ MODO CONVERSACIONAL (NUEVO)
      // Responde preguntas normales antes de caer en bienvenida/cooldown.
      const conversationalReply = await buildConversationalReply({
        userText,
        session,
        customerName: customerName || userPhone,
      });

      if (conversationalReply) {
        debugJson("💬 Respuesta conversacional lista", {
          userPhone,
          msgId,
          type: conversationalReply.type || "text",
          hasCta: Boolean(conversationalReply.url),
          bodyPreview: String(conversationalReply.body || "").slice(0, 240),
        });
        await sendConversationalReply({
      ...
      if (priceIntent && referredProduct?.name) {
        ...
        const priceWelcomeText = buildPriceWelcomeConversationalText(greetingName, referredProduct);
      ...
      const welcomeText = buildWelcomeConversationalText(greetingName, referredProduct);
